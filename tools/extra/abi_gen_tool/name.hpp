...
namespace std {
   template<> struct hash<eosio::name> : private hash<uint64_t> {
      typedef eosio::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const eosio::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, eosio::name& check);
} // fc
